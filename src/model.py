...
from src.runtime_device import DEVICE, autocast
from src.spectral_perspective_loss import spectral_perspective_loss
device = DEVICE
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    collect_result_summary: bool = field(default=False, metadata={"help": "Accepted for compatibility; the minimal release does not build aggregate summaries."})
    save_latents: bool = field(default=True, metadata={"help": "Save per-example inference latent states during evaluation."})
    ...
    print_loss: bool = field(default=False, metadata={"help": "Print loss information."})
    max_token_num: int = field(default=1000, metadata={"help": "Limit the longest data to avoid OOM."})
    spectral_loss_factor: float = field(default=0.0, metadata={"help": "Weight for spectral perspective regularization on latent tokens. 0 disables the method."})
...
def get_steps(
    ...
    if qwen_tokenizer is not None:
        newline_ids = qwen_tokenizer.encode("\n", add_special_tokens=False)
        if len(newline_ids) != 1:
            raise ValueError(f"Qwen newline delimiter must be one token, got {newline_ids!r}")
        newline_id = newline_ids[0]
        eot_id = qwen_tokenizer.eos_token_id
        pad_id = qwen_tokenizer.pad_token_id
        stop_set = {eot_id, pad_id}
        ...
        for b in range(B):
            ...
            for idx, tok in enumerate(seq):
                ...
                if tok not in token_cache:
                    token_cache[tok] = qwen_tokenizer.convert_ids_to_tokens(tok) or ""
                token_text = token_cache[tok]
                if "\n" in token_text or "Ċ" in token_text:
                    newline_positions.append(idx)

            steps_for_sample: List[List[int]] = []
            for start, end in zip(newline_positions, newline_positions[1:]):
                if end > start + 1:
                    steps_for_sample.append(seq[start + 1:end + 1] + [eot_id])
...
class CODI(torch.nn.Module):
    def __init__(self, model_args, training_args, lora_config):
        ...
        _dtype = torch.float16 if training_args.bf16 is False else torch.bfloat16
        ...
        # Spectral Perspective Loss
        self.spectral_loss_factor = training_args.spectral_loss_factor
    ...
    def forward(
        ...
        if self.model_args.use_decoder:
            ...
            if 'llama' in self.model_args.model_name_or_path.lower():
                ...
                steps_list = get_steps(ref_input_ids, self.num_latent+1, qwen_tokenizer=self.tokenizer)
                steps_pad_list = pad_steps(steps_list, pad_id=self.tokenizer.pad_token_id)
        ...
        collect_latents_for_spectral_loss = self.spectral_loss_factor > 0
        latent_embeddings_for_spectral_loss = []
        if self.num_latent != 0:
            if collect_latents_for_spectral_loss:
                latent_embeddings_for_spectral_loss.append(latent_embd.detach().clone() if not self.training else latent_embd)

            for i in range(num_latent):
                current_latent_input = latent_embd

                with autocast(dtype=torch.bfloat16):
                    outputs = self.codi(inputs_embeds=current_latent_input, use_cache=True, output_hidden_states=True, past_key_values=past_key_values)
                ...
                if collect_latents_for_spectral_loss:
                    latent_embeddings_for_spectral_loss.append(latent_embd)
        ...
        # Spectral perspective loss on latent reasoning tokens
        spectral_loss = torch.tensor(0.0, device=encoder_input_ids.device)
        if len(latent_embeddings_for_spectral_loss) > 1:
            latent_matrix = torch.cat(latent_embeddings_for_spectral_loss, dim=1)  # (B, N, D)
            batch_spectral_loss = torch.tensor(0.0, device=latent_matrix.device)
            for b in range(latent_matrix.size(0)):
                batch_spectral_loss = batch_spectral_loss + spectral_perspective_loss(latent_matrix[b])
            spectral_loss = (batch_spectral_loss / latent_matrix.size(0)) * self.spectral_loss_factor

        if self.print_loss:
            spectral_str = f', spectral_loss={spectral_loss.item():.4f}' if self.spectral_loss_factor > 0 else ''
            if self.model_args.use_decoder:
                print(f'loss={ce_loss+distill_loss}, ce_loss={ce_loss}, distill_loss={distill_loss}, ce_loss_total={ce_loss_total}, distill_loss_total={distill_loss_total}, ref_ce_loss={ref_ce_loss}, explain_loss={explain_loss_total}{spectral_str}')
            else:
                print(f'loss={ce_loss+distill_loss}, ce_loss={ce_loss}, distill_loss={distill_loss}, ce_loss_total={ce_loss_total}, distill_loss_total={distill_loss_total}, ref_ce_loss={ref_ce_loss}{spectral_str}')

        loss = ce_loss_total + distill_loss_total + ref_ce_loss + spectral_loss
        ...
        result = {"loss": loss, "logits": logits, "ce_loss": ce_loss_total, "distill_loss": distill_loss_total, "ref_ce_loss": ref_ce_loss, "spectral_loss": spectral_loss}
        if self.model_args.use_decoder:
